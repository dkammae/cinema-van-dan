(async function () {
  ...
  if (!name) {
    loading.textContent = 'Geen cast lid opgegeven.';
    return;
  }

  let data;
  try {
    const res = await fetch(`/api/cast/${encodeURIComponent(name)}`);
    if (!res.ok) throw new Error(`API error ${res.status}`);
    data = await res.json();
  } catch (e) {
    loading.textContent = `Kon cast lid niet laden: ${e.message}`;
    return;
  }
  ...
  root.innerHTML = `
    <div class="director-hero">
      ...
      <div class="director-hero-body">
        <span class="section-kicker">Cast</span>
            ...
            <a class="director-film-card" href="film.html?id=${encodeURIComponent(f.id)}">
              ...
              <div class="director-film-body">
                ...
                <div class="meta">
                  ...
                  ${f.directors?.length ? `<span class="dir">regie ${escapeHTML(f.directors[0])}</span>` : ''}
                </div>
                ${f.description ? `<p>${escapeHTML(truncate(f.description, 100))}</p>` : ''}
                ${f.awards?.length ? `<ul class="film-card-awards">${f.awards.slice(0, 3).map(a => `<li class="${a.status==='win'?'win':'nom'}">${a.status==='win'?'🏆':'✦'} ${escapeHTML(a.category)} <span class="award-meta">— ${escapeHTML(a.ceremony)} ${escapeHTML(a.year)}</span></li>`).join('')}${f.awards.length > 3 ? `<li class="more">+${f.awards.length - 3} meer</li>` : ''}</ul>` : ''}
              </div>
            </a>
          `).join('')}
        </div>
      ` : '<p class="empty">Geen films met deze cast in je collectie.</p>'}
    </section>
  `;
